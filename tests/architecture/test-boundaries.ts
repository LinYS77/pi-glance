...
function assertRenderPathsStayIoFree(files: SourceFile[]): void {
	...
	for (const file of files.filter((candidate) => RENDER_MODULES.has(candidate.path) || candidate.path.startsWith("src/segments/"))) {
		for (const record of importsFrom(file)) {
...
function assertHighValueImportRules(files: SourceFile[]): void {
	...
	const state = files.find((file) => file.path === "src/runtime/state.ts");
	...
	const runtime = files.find((file) => file.path === "src/runtime/runtime.ts");
	assert.ok(runtime, "runtime.ts should exist");
	const forbiddenRuntimeImports = new Set(["../surface/frame.js", "../surface/layout.js", "../surface/status-line.js", "../surface/renderer.js", "../settings/pane.js", "../segments/render.js", "./state.js", "./snapshot.js"]);
...
function assertNativeTestRunner(packageText: string): void {
	...
	assert.ok(testDev.includes(".tmp-test/tests/*/test-*.js"), "test:dev should discover compiled tests through one glob");
}
...
test("pure models stay transitively free of IO and Pi runtime dependencies", () => {
	...
	assert.equal(dependencyPath(files, "src/runtime/state.ts", (specifier) => IO_NETWORK_PROCESS_IMPORTS.has(specifier)), undefined);
});

test("render imports stay transitively free of IO outside the Pi host", () => {
	for (const file of files.filter((file) => RENDER_MODULES.has(file.path) || file.path.startsWith("src/segments/"))) {
