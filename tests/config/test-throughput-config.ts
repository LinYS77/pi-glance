...
import { CONFIG_VERSION, cloneConfig, configFromText, configToText, defaultConfig, normalizeConfig } from "../../src/config/model.js";
...
assert.equal(defaults.version, CONFIG_VERSION, "defaultConfig should use the current config schema version");
assert.equal(normalizeConfig({ version: 0 }).version, CONFIG_VERSION, "old raw versions should normalize to the current schema");
assert.equal(normalizeConfig({ version: 999 }).version, CONFIG_VERSION, "future raw versions should normalize to the current schema");
...
assert.equal(JSON.parse(encoded).version, CONFIG_VERSION, "configToText should serialize the current schema");
