...
test("config store diagnoses reads and atomically saves through an explicit path", async () => {
	...
	try {
		...
		const futureRaw = { version: 99, enabled: false, icons: "nerd", futureOnly: { preserve: true } };
		await writeConfigText(configPath, JSON.stringify(futureRaw));
		const futureExpected = normalizeConfig(futureRaw);
		assertLoadResult(
			loadConfigSync(),
			{ config: futureExpected, status: "future", writable: false, diagnostic: /version 99.*newer than supported version 12.*without overwriting/i },
