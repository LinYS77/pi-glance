...
import { getSettingsCategories, getSettingsRows } from "../src/settings/catalog.js";
...
assert.deepEqual(
	...
	"Model speed settings should be exactly Enabled + Precision",
);

assert.equal(rows.some((row) => /notify/i.test(`${row.id} ${row.label} ${row.hint}`)), false, "Model speed settings should not expose a notify row");
assert.equal(rows.some((row) => /Metric|Window|Includes/.test(row.label)), false, "Model speed settings should remove old Metric/Window/Includes info rows");
assert.equal(rows[0]?.apply !== undefined, true, "Model speed Enabled row should be editable");
assert.equal(rows[1]?.apply !== undefined, true, "Model speed Precision row should be editable");

const before = clone(config);
const afterToggle = rows[0]!.apply!(config) as GlanceConfig;
assert.deepEqual(config, before, "Model speed Enabled row should not mutate the input config");
assert.equal(afterToggle.segments.find((segment) => (segment as SegmentConfigLike).id === "throughput")?.enabled, false, "Model speed Enabled row should toggle throughput segment off");
assert.equal(getSettingsRows(afterToggle, "throughput" as never)[0]?.value, "off", "Model speed Enabled row value should reflect disabled state after toggle");

const autoToOne = rows[1]!.apply!(config) as GlanceConfig;
assert.equal(precisionOf(autoToOne), 1, "Model speed Precision should cycle auto -> 1 digit");
...
assert.equal(precisionOf(oneToZero), 0, "Model speed Precision should cycle 1 digit -> 0 digits");
...
assert.equal(precisionOf(zeroToAuto), "auto", "Model speed Precision should cycle 0 digits -> auto");
