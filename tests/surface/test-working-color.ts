...
test("changing sweep color changes only the active beam, in both animation modes", () => {
	for (const mode of ["top", "perimeter"] as const) {
		...
		const config = defaultConfig(); config.editor.activityMode = "sweep"; config.editor.workingSweep = mode;
		const changed = { ...config, editor: { ...config.editor, workingSweepColor: "blue" as const } };
		const idle = render(config, 220);
		const original = render(config, 220, { animation: { kind: "sweep", elapsedMs: 1200, speed: 47 } });
		const blue = render(changed, 220, { animation: { kind: "sweep", elapsedMs: 1200, speed: 47 } });
...
test("live and preview frames agree for every color, with no stale colors when resized or changed back", () => {
	...
	let config = defaultConfig(); config.editor.activityMode = "sweep";
	let now = 0;
	...
	const editor = new GlanceEditor({ terminal: { rows: 24 }, requestRender() {} } as unknown as TUI,
		...
		{ animationNowMs: () => now, scheduleAnimationFrame: () => () => {}, getExtensionStatuses: () => statuses });
	editor.focused = true; editor.setText("untouched input");
	editor.setWorkingStatusIndicator(nativeActivity("working")); editor.render(220); now = 1200;
	for (const mode of ["top", "perimeter"] as const) {
		for (const color of [...WORKING_SWEEP_COLOR_VALUES, "theme"] as const) {
			...
			for (const width of [220, 80, 1, 220]) {
				...
				const preview = render(config, width, { animation: { kind: "sweep", elapsedMs: 1200, speed: 47 }, extensionStatuses: statuses });
