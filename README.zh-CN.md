...
- **Working 扫光** — Pi 工作时，扫光绕框或沿顶部移动，可调速度，并提供 9 种与主题搭配的配色选项。
...
运行 `/glance`，设置 **Appearance（外观）**、**Status line（状态栏）**、**Working（工作动画）** 和 **Input（输入）**。`Tab` 切换分区，方向键调整，`S` 保存。
...
- 需要 Pi 0.85.0+、Node.js 22.19.0 或更高版本，建议使用 Pi 0.85.1。
