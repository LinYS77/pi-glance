...
for (const match of throughput.text.matchAll(importPattern)) {
	...
	if (specifier.startsWith("@earendil-works/pi-")) fail(`${throughput.path}: throughput pure module must not import pi package ${specifier}`);
	if (IO_NETWORK_PROCESS_IMPORTS.has(specifier)) fail(`${throughput.path}: throughput pure module must not import IO/network/process module ${specifier}`);
	if (["./runtime.js", "./renderer.js", "./status-line.js", "./pane.js", "./editor.js", "./config.js", "./settings-catalog.js"].includes(specifier)) {
		fail(`${throughput.path}: throughput pure module must not import UI/runtime/config module ${specifier}`);
	}
...
if (/\bDate\.now\s*\(/.test(throughput.text)) fail(`${throughput.path}: throughput calculation must use injected timestamps, not Date.now()`);
if (/\.notify\s*\(/.test(throughput.text)) fail(`${throughput.path}: throughput calculation must never notify`);
if (/(?:\.\s*(?:content|delta|text_delta|thinking_delta)\b|\[\s*["'](?:content|delta|text_delta|thinking_delta)["']\s*\])/.test(throughput.text)) {
	fail(`${throughput.path}: throughput calculation must not read message content/delta text as a token fallback`);
}
if (/\.\s*length\b/.test(throughput.text)) {
	fail(`${throughput.path}: throughput calculation must not use string/content length as a token fallback`);
}
...
for (const fileName of ["throughput.ts", "runtime.ts", "segment-registry.ts", "renderer.ts", "status-line.ts"] as const) {
...
for (const fileName of ["throughput.ts", "runtime.ts", "segment-registry.ts"] as const) {
	const file = byPath.get(fileName);
