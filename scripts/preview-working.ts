...
const state: GlanceState = {
	...
	git: { repo: true, branch: "main", detached: false, sha: "abcdef1", upstream: "origin/main", ahead: 1, behind: 0, staged: 0, unstaged: 1, untracked: 0, conflicts: 0, dirty: true, status: "dirty", updatedAt: 0 },
	...
	throughput: {
		lastRun: { startedAtMs: 0, endedAtMs: 1000, elapsedMs: 1000, tokensPerSecond: 43, usage: { input: 0, output: 43, cacheRead: 0, cacheWrite: 0, totalTokens: 43, assistantMessages: 1 } },
		currentRun: null,
	},
	version: 0,
};
...
tui.addChild({
	...
	handleInput(data) {
		...
		if (matchesKey(data, "space")) {
			...
			warnings = !warnings;
			state.context.percent = warnings ? 92 : 24;
			state.context.tokens = warnings ? 625_600 : 163_200;
			state.git.status = warnings ? "conflict" : "dirty";
