...
- **Tokens** — input / output or total; Cache offers `rate` (default `CH%`), `read/write` (actual `R… W…` token amounts), or `hide`. Tokens stay off by default.
